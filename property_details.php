...
<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        $checkout_date = explode('-', $_POST['checkout_date']);
        $checkin_day = (int)$checkin_date[2];
        $checkout_day = (int)$checkout_date[2];
        if ($checkin_day >= $checkout_day) {
            $err = "invalid dates: checkin_date >= checkout_day";
        }

        for($i = $checkin_day; $i <= $checkout_day; $i++):
            if ($available_list[$i-1] == 'f'){
                $err = "invalid dates: property already booked!!";
                break;
            }
        endfor;
...
<body id = 'property_detials'>
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])."?property_id=".$id ; ?>" method="post">
            ...
            <h2>BOOKING DATES:</h2>
            Check-in Date: <input type="date" id="checkin_date" name="checkin_date" min="2020-03-01" max="2020-03-31">
            Check-out Date: <input type="date" id="checkout_date" name="checkout_date" min="2020-03-01" max="2020-03-31">

            <input type="submit" class="book" id = "book" value="Book">
